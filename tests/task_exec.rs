...
use mantissa::task::docker::{
    ContainerCreateRequest, ContainerError, ContainerExecOptions, ContainerExecResult,
    ContainerInfo, ContainerLogFrame, ContainerLogStream, ContainerManager,
};
...
#[derive(Clone)]
struct CollectingTaskExecSink {
    frames: CapturedTaskFrames,
    ended: Arc<Notify>,
}

impl Default for CollectingTaskExecSink {
    fn default() -> Self {
        Self {
...
impl task_log_sink::Server for CollectingTaskExecSink {
    async fn push_frame(
        self: std::rc::Rc<Self>,
...
local_test!(task_exec_relay_over_tcp_sessions, {
    let owner_manager = Arc::new(StaticExecContainerManager::default());
    let requester_manager = Arc::new(StaticExecContainerManager::default());
    ...
    let _guard = ContainerManagerOverrideGuard::install_factory(Arc::new(move || {
        match index_for_factory.fetch_add(1, Ordering::SeqCst) {
            ...
            _ => Arc::new(StaticExecContainerManager::default()),
        }
    }));
    ...
    let owner = match TestNode::try_new_tcp_with_tick_ms(100).await {
        ...
        Err(err) => {
            ...
            if msg.contains("Operation not permitted") {
                eprintln!("skipping task_exec_relay_over_tcp_sessions: {msg}");
                return;
            }
    ...
    let sink = CollectingTaskExecSink::default();
    ...
    let response = request
        ...
        .promise
        .await
        .expect("exec relay request should succeed");
    ...
    input.get().set_data(b"echo cluster exec\n");
    input.send().await.expect("send exec input");
    session
        .close_input_request()
        .send()
        .promise
    ...
    let wait = session
        ...
        .promise
        .await
        .expect("wait for exec result");
    let wait = wait.get().expect("exec result payload");
    assert!(wait.get_has_exit_code());
    assert_eq!(wait.get_exit_code(), 0);
    ...
    {
        panic!(
            "exec stream should end: calls={:?} inputs={:?} frames={:?}",
    ...
    assert_eq!(
        sink_frames.lock().await.clone(),
        vec![
            ("console".to_string(), b"/ # ".to_vec()),
            ("stdout".to_string(), b"exec ready\n".to_vec()),
        ]
    );
});
