...
pub type ServiceStoreInner =
    CrdtMstStore<MvRegAdapterSorted<UuidKey, ServiceSpecValue, Uuid>, XXHash128, ServiceTables>;
...
pub fn open_service_store(db: Arc<redb::Database>, actor: Uuid) -> std::io::Result<ServiceStore> {
    let inner = ServiceStoreInner::open(db, actor)?;
    Ok(Arc::new(inner))
}
