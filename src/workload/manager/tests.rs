...
use crate::workload::model::{
    ExecutionSubstrate, WorkloadOwner, WorkloadServiceMetadata, WorkloadStatus, WorkloadValue,
    WorkloadValueDraft,
};
...
#[tokio::test]
async fn start_tasks_batch_rejects_unsupported_local_execution_substrate() {
    let (manager, scheduler, _mock_cm, _network_registry) = setup_manager().await;
    scheduler
        .init_slots(vec![SlotSpec::new(
            1,
    ...
    match cause {
        SchedulingError::RuntimeRequirementsBlocked {
            ...
            assert_eq!(*execution_substrate, "microvm");
